...
// fonction qui recupere la liste des utxo
const getUtxoId=async (amount)=>{
    let adress='1DnKzNhp5JQyQRNPHP4coekGcA7KpnPzji'
    let utxos= await fetch('https://blockchain.info/unspent?active='+adress, {method: 'GET'})
...
const parseUtxo=(amount, utxos)=>{
    ...
    if(todo) {  // on utilise plusieurs entrees
        ...
        for(i=0; i<second.length-1; i++) {
            ...
            for(j=0; j<second.length; j++) {
                if (i===j) {continue}
                    ...
                    if((second[i].value+second[j].value)>=(amount+7500)){
                        ind={i: i, j: j}
                        witness=true
                        break
                    }
        ...
        if(witness) {
            ...
            i=ind.i
            j=ind.j
            if(second[i].value+second[j].value>=amount+9000){
                todo=false
                return {
                    status: 'success',
                    input: 2,
    ...
    if(todo) {
        console.log("on a passe a 3 entrees")
        if(amount>=490000) {
                ...
                for(j=0; j<third.length-1; j++) {
                    ...
                    if (i===j || i===k || j===k) {continue}
                        ...
                        for(k=0; k<third.length; k++){
                            if(witness){break}
                            if (i===k || j===k) {continue}
                            if((third[i].value+third[j].value+third[k].value)>=(amount+10000)){
                                witness=true
                                ind={i: i, j: j, k: k}
            ...
            if(witness) {
                ...
                j=ind.j
                k=ind.k
                if(third[i].value+third[j].value+third[j].value>=amount+12000){
                    todo=false
                    return {
                        status: 'success',
                        input: 3,
    ...
    if(todo) return {status: 'fail', cause: 'need too many entries'}
}
